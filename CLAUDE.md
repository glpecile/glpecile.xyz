...
- Quick commands: `bun run check` (astro type check), `bun run lint` (biome), `bun run test` (vitest unit tests), `bun run build` (production build). Run `check` after any `.astro` or content-schema change.
