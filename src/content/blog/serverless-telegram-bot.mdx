...
Okay. This is the piece that got me my *Odyssey* tickets, so let's give it its due.
...
## Three things you'll hit no matter what

None of these are architecture. All of them cost me real time, and all of them are cheaper on day one.
...
**Your shell history is not a secrets manager.** Bun auto-loads `.env`, `.env.local`, and `.env.[NODE_ENV]` for `bun run` — no dotenv package, no code changes. Real values go in a gitignored `.env.local`, placeholders in a committed `.env.template`, and the setup command collapses from `TOKEN=... URL=... SECRET=... bun run webhook` to `bun run webhook`. (Deploy-side secrets still belong in the platform's secret store.)
